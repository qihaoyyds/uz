...
//@env:UCCookie##用于播放UC网盘视频&&UC_UT##播放视频自动获取，不可用时点击删除重新获取 cookie ，再重启app&&夸克Cookie##用于播放Quark网盘视频&&阿里Token##用于播放阿里网盘视频&&转存文件夹名称##在各网盘转存文件时使用的文件夹名称&&123网盘账号##用于播放123网盘视频&&123网盘密码##用于播放123网盘视频&&天翼网盘账号##用于播放天翼网盘视频&&天翼网盘密码##用于播放天翼网盘视频&&百度Cookie##用于播放百度网盘视频&&BDUSS##百度账号认证信息&&STOKEN##百度账号安全令牌
...
class BaiduPan {
    ...
    async validateCookie() {
        try {
            const resp = await req('https://pan.baidu.com/api/user/info', {
    ...
    async getFilesByShareUrl(shareUrl) {
        ...
        try {
            ...
            for (const video of videos) {
                ...
                videoItem.panType = PanType.BaiduPan;
    ...
    async getShareInfo(shareKey, pwd = '') {
        try {
            const resp = await req(`${this.shareApiUrl}verify?surl=${shareKey}`, {
                method: 'POST',
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
    ...
    async getFileList(shareKey, uk, shareid, dir) {
        try {
            const resp = await req(`${this.shareApiUrl}list?uk=${uk}&shareid=${shareid}&dir=${encodeURIComponent(dir)}&surl=${shareKey}`, {
                headers: {
                    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36',
                    'Referer': `https://pan.baidu.com/s/${shareKey}`
                },
    ...
    async getDownloadLink(fileData) {
        try {
            const resp = await req(`${this.apiUrl}sharedownload?app_id=250528&channel=chunlei&clienttype=12&sign=${fileData.sign}&timestamp=${fileData.timestamp}&web=1`, {
                query: {
                    encrypt: 0,
                    product: 'share',
                    uk: fileData.uk,
...
class PanTools {
    ...
    async registerRefreshAllCookie() {
        ...
        async getAllCookie() {
            this.baidu.cookie = (PanType.baidu, this.cookie)
        }
    }
    ...
    async getPlayInfo(item) {
        ...
        if (item.panType === PanType.Quark) {
            ...
            return JSON.stringify(data)
        } else if (item.panType === PanType.BaiduPan) {
            return this.baidu.getPlayUrl(item);
            return JSON.stringify(data)
        }
